...
if args.optimizer is 'SGD':
    results_save_path = './Results_Sphere_Torus/input_dim_{5}/init_{6}/layers_{0}/neuron_{1}/lambda_cvx_{10}_mean_{11}/optim_{8}lr_{2}momen_{7}/gen_{9}/batch_{3}/trial_{4}_last_{12}_qudr'.format(
    ...
    results_save_path = './Results_Sphere_Torus/input_dim_{5}/init_{6}/layers_{0}/neuron_{1}/lambda_cvx_{11}_mean_{12}/optim_{9}lr_{2}betas_{7}_{8}/gen_{10}/batch_{3}/trial_{4}_last_{13}_qudr'.format(
...
get_sphere_samples = functools.partial(get_volume_samples, condition=torus_condition)
get_torus_samples = functools.partial(get_volume_samples, condition=sphere_condition)

X_data = get_torus_samples(60000)
print("generated torus samples")
...
y_plot = Variable(get_sphere_samples(args.N_PLOT), requires_grad=True)
...
def train(epoch):
    ...
    for batch_idx, real_data in enumerate(train_loader):
        ...
        y = Variable(get_sphere_samples(args.BATCH_SIZE), requires_grad= True)
